...
int main(int argc, char *argv[]) {
	...
	int total_ta_time, total_process_count;
	...
	total_ta_time = 0; total_process_count = 0;
	interval_time = atoi(argv[2]);
	
	if(argc == 4) output_filename = argv[3];
	else output_filename = "SPN_out.csv";
	...
	while (1) {
		...
		while (1) {
			...
			// 프로세스가 처리 완료되기 전에 프로세스가 추가로 들어오는 경우
			...
			total_ta_time += now_time - write_buffer.entry_time;
			total_process_count++;
	...
	double SPN_nomalized_ATAT = nomalized_ATAT(total_ta_time, total_process_count);
	printf("SPN_평균반환시간 = %.2f\n", SPN_nomalized_ATAT);
